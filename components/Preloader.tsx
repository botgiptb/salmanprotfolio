...
export default function Preloader() {
  ...
  // 1) After hydration, decide whether to show the preloader
  useEffect(() => {
    setMounted(true);
    ...
    // Skip if already played this browser session
    try {
  ...
  // 2) Once visible, run the counter
  useEffect(() => {
    if (!visible) return;
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <div className="absolute bottom-8 left-8 w-8 h-8 border-b-2 border-l-2 border-brand-cyan/40 rounded-bl" />
          <div className="absolute bottom-8 right-8 w-8 h-8 border-b-2 border-r-2 border-brand-purple/40 rounded-br" />
          ...
          <div className="relative w-full max-w-[500px] px-8 flex flex-col items-center gap-10">
            ...
            <div className="w-full flex flex-col items-center gap-4">
              ...
              {/* Percentage + status side by side */}
              <div className="w-full flex justify-between items-baseline px-1">
                <span className="font-heading text-[10px] font-bold tracking-[0.2em] uppercase text-zinc-500">
                  {statusText}
                </span>
              ...
              <div className="w-full h-[2px] bg-zinc-900 rounded-full overflow-hidden relative">
                ...
                <motion.div
                  ...
                  className="h-full bg-gradient-to-r from-brand-purple via-violet-400 to-brand-cyan rounded-full relative transition-all duration-75 ease-linear"
                >
